...
private:
  ...
  void * entry(void *) {
    ...
    alloc_config[2].block_size = sizeof(job_descriptor_t);
		...
		alloc_config[3].block_size = sizeof(struct pbuf);
		alloc_config[3].num_blocks = _pbuf_num_per_core * _rx_threads_per_nic;
		alloc_config[3].alignment = 64;
		alloc_config[3].allocator_id = PBUF_ALLOCATOR;

    alloc_config[4].block_size = sizeof(Frames_list_node);
    alloc_config[4].num_blocks = _pbuf_num_per_core * _tx_threads_per_nic;
    alloc_config[4].alignment = 64;
    alloc_config[4].allocator_id = FRAME_LIST_ALLOCATOR;
...
*/
    _tx_queues_per_nic = NUM_CHANNEL * 2;
    _tx_desc_per_queue = NUM_TX_DESCRIPTORS_PER_QUEUE;
    _rx_threads_per_nic = NUM_CHANNEL;
    _tx_threads_per_nic = NUM_CHANNEL;
    _frame_num_per_core = NUM_FRAME_PER_CORE;
    _jd_num_per_core = NUM_JD_PER_CORE;
    _mbuf_num_per_core = NUM_EXO_MBUF_PER_CORE;
    _meta_data_num_per_core = NUM_META_DATA_PER_CORE;
    _pbuf_num_per_core = NUM_PBUF_PER_CORE;
    _net_header_num_per_core = NUM_NET_HEADER_PER_CORE;
    _ip_reass_num_per_core = NUM_IP_REASS_PER_CORE;
    _udp_pcb_num_per_core = NUM_UDP_PCB_PER_CORE;
